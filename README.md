...
**集約**は、一緒に整合性を保つオブジェクトのまとまり。

- 全体(集約ルート)が部分(集約境界内のオブジェクト)を包含する
- 外部から境界内オブジェクトを操作するときは、集約ルートを通す

全体-部分の関係の強さは菱形で表す。見分け方は「全体が消えたとき、部分は生き残るか」。

- **コンポジション(黒菱形)**: 部分は全体なしでは存在できない。User と UserName
- **集約(白菱形)**: 部分は単独で生き残る。Circle と User
...
「直接の友達とだけ話しなさい。見知らぬ他人に話しかけてはいけない」。
オブジェクト指向のガイドラインで、結合度を下げて変更に強くする。別名**最小知識の原則**。
...
```go
// NG: Circle の中の User を取り出して操作している
circle.Members()[0].ChangeName(newName)
if len(circle.Members()) >= 30 {
}
```

User の名前の持ち方が変わると、Circle を触っている側まで壊れる。

### 従った例（用事がある相手に頼む）

中身は覗かず、友達に頼み事をする。メンバー追加も満員判定も Circle に聞く。名前の変更は User に頼む。
...
`Join` の中で `c.members` を触るのは問題ない。Circle 自身のフィールドだから。

### メソッドチェーンのすべてが違反ではない

ドットが続くこと自体がダメなのではない。別種のオブジェクトを芋づる式に手繰るのが違反。

```go
// NG: Circle から User を手繰って操作している
circle.Members()[0].ChangeName(newName)
```

同じ Circle への依頼が続く分には問題ない。
